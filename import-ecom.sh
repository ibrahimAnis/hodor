...
# Clients
./bin/kcadm.sh create clients -r master -s clientId=ecom-portal -s publicClient=true -s 'redirectUris=["http://localhost/oauth2/callback"]' -s 'webOrigins=["http://localhost/","http://localhost:3000", "http://todo-api:8000" ]' -s 'attributes={ "post.logout.redirect.uris": "http://localhost:3000", "access.token.lifespan": 3600}' -o
